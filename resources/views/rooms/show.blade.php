...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <div class="d-flex align-items-center mb-1">
                <h2 class="fw-bold text-dark mb-0 me-3">{{ $room->name }}</h2>
                @if($room->is_occupied)
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 small">
                    ...
                    <li class="breadcrumb-item active text-muted">{{ $room->name }}</li>
                </ol>
            </nav>
        </div>
        <div class="d-flex gap-2">
            @can('update', $room)
                <a href="{{ route('rooms.edit', $room) }}" class="btn btn-outline-primary">
    ...
    <div class="row">
        {{-- Colonne principale --}}
        <div class="col-lg-8">
            {{-- Carte principale avec section dédiée à la photo et aux informations --}}
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                    ...
                    @can('create', App\Models\Meeting::class)
                        <a href="{{ route('meetings.create', ['room_id' => $room->id]) }}" class="btn btn-sm btn-outline-primary">
        ...
        <div class="col-lg-4">
            {{-- Statut actuel --}}
            <div class="card border-0 shadow-sm mb-4 {{ $room->is_occupied ? 'border-danger' : ($room->is_active ? 'border-success' : 'border-secondary') }}" style="border-width: 2px !important;">
                    ...
                    @if($room->is_occupied)
                        ...
                        @if($room->current_meeting)
                            ...
                            <p class="small text-muted">
                                Jusqu'à {{ $room->current_meeting->end_at?->format('H:i') ?? $room->current_meeting->start_at?->copy()->addMinutes($room->current_meeting->duration_minutes ?? 60)->format('H:i') }}
                            </p>
                        @endif
                    @elseif(!$room->is_active)
            ...
            @if($room->next_meeting && !$room->is_occupied)
                <div class="card border-0 shadow-sm mb-4 bg-light">
                    <div class="card-body">
                        ...
                        <a href="{{ route('meetings.show', $room->next_meeting) }}" class="btn btn-sm btn-outline-primary mt-3">
                            Voir les détails
                        </a>
                    </div>
                </div>
            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        @can('create', App\Models\Meeting::class)
                            <a href="{{ route('meetings.create', ['room_id' => $room->id]) }}" 
                               class="btn btn-primary {{ $room->is_occupied || !$room->is_active ? 'disabled' : '' }}">
                        ...
                        @can('update', $room)
                            ...
                            <form action="{{ route('rooms.toggle-status', $room) }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-outline-{{ $room->is_active ? 'warning' : 'success' }} w-100">
