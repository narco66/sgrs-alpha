...
<div class="modern-card mb-4">
    <div class="modern-card-body">
        <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" id="documentUploadForm">
            @csrf
            ...
            <div class="row g-3 mt-3 d-none" id="documentDetails">
                ...
                <div class="col-md-12">
                    ...
                    <select name="meeting_id" class="form-select @error('meeting_id') is-invalid @enderror">
                        <option value="">Aucune réunion</option>
                        @foreach($meetings ?? [] as $meeting)
                            <option value="{{ $meeting->id }}">{{ $meeting->title }} - {{ $meeting->start_at?->format('d/m/Y') }}</option>
                        @endforeach
                    </select>
                    @error('meeting_id')
                        <div class="invalid-feedback">
...
<div class="modern-filters mb-4" style="background: linear-gradient(135deg, #ffffff 0%, #f8fafc 100%); border-radius: 20px; padding: 2rem; border: 2px solid #f1f5f9; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.05);">
    <div class="row g-3 align-items-end">
        ...
        <div class="col-md-6">
            <div class="d-flex justify-content-end gap-2 align-items-end">
                ...
                <a href="{{ route('document-types.index') }}" class="btn btn-modern btn-modern-secondary" style="border-radius: 12px;">
    ...
    <div class="mt-4 pt-3" style="border-top: 2px solid #f1f5f9;">
        ...
        <div class="document-type-filters">
            <a href="{{ route('documents.index', array_merge(request()->except(['ext', 'page']), ['ext' => 'all'])) }}" 
               class="filter-pill filter-pill-all {{ ($extension ?? 'all') === 'all' ? 'active' : '' }}">
                <i class="bi bi-grid-3x3-gap"></i>
                Tous
            </a>
            <a href="{{ route('documents.index', array_merge(request()->except(['ext', 'page']), ['ext' => 'pdf'])) }}" 
               class="filter-pill filter-pill-pdf {{ ($extension ?? 'all') === 'pdf' ? 'active' : '' }}">
                <i class="bi bi-file-earmark-pdf"></i>
                PDF
            </a>
            <a href="{{ route('documents.index', array_merge(request()->except(['ext', 'page']), ['ext' => 'word'])) }}" 
               class="filter-pill filter-pill-word {{ in_array($extension ?? '', ['doc', 'docx']) ? 'active' : '' }}">
                <i class="bi bi-file-earmark-word"></i>
                Word
            </a>
            <a href="{{ route('documents.index', array_merge(request()->except(['ext', 'page']), ['ext' => 'powerpoint'])) }}" 
               class="filter-pill filter-pill-powerpoint {{ in_array($extension ?? '', ['ppt', 'pptx']) ? 'active' : '' }}">
                <i class="bi bi-file-earmark-ppt"></i>
                PowerPoint
            </a>
            <a href="{{ route('documents.index', array_merge(request()->except(['ext', 'page']), ['ext' => 'excel'])) }}" 
               class="filter-pill filter-pill-excel {{ in_array($extension ?? '', ['xls', 'xlsx']) ? 'active' : '' }}">
        ...
        @foreach($documents as $document)
            <div class="col-md-4 col-lg-3">
                <div class="document-card-image-style">
                    <div class="document-icon-square {{ strtolower($document->extension) }}">
                        <span class="document-icon-text">
                            @if(strtolower($document->extension) === 'pdf')
                                PDF
                            @elseif(in_array(strtolower($document->extension), ['doc', 'docx']))
                                W
                            @elseif(in_array(strtolower($document->extension), ['xls', 'xlsx']))
                                X
                            @elseif(in_array(strtolower($document->extension), ['ppt', 'pptx']))
                                P
                            @else
                                {{ strtoupper(substr($document->extension, 0, 1)) }}
                            @endif
                        </span>
                    </div>
                    <div class="document-card-content">
                        <h6 class="document-title-image" title="{{ $document->title }}">
                            {{ Str::limit($document->title, 45) }}
                        </h6>
                        <div class="document-details-image">
                            {{ number_format($document->file_size / 1024 / 1024, 1, ',', ' ') }} MB - 
                            {{ $document->created_at?->format('d/m/Y') }} - 
                            {{ $document->uploader?->first_name ?? '' }} {{ $document->uploader?->last_name ?? $document->uploader?->name ?? 'N/A' }}
                        </div>
                        @if($document->type)
                            @php
                                $typeColors = [
                            ...
                            @endphp
                            <div class="document-type-badge-image" style="background: {{ $colors['bg'] }}; color: {{ $colors['text'] }};">
                                {{ $typeName }}
                            </div>
                        @endif
                    </div>
    ...
    @if($documents->hasPages())
        <div class="modern-card-footer mt-4">
            <div class="small text-muted">
                ...
                sur {{ $documents->total() }} document{{ $documents->total() > 1 ? 's' : '' }}
            </div>
