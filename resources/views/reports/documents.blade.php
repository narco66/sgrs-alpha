...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="btn-group">
        <a href="{{ route('reports.export', ['documents', 'pdf']) }}?start_date={{ $startDate->format('Y-m-d') }}&end_date={{ $endDate->format('Y-m-d') }}" 
           class="btn btn-outline-danger" target="_blank">
            <i class="bi bi-file-pdf"></i> Export PDF
        </a>
        <a href="{{ route('reports.export', ['documents', 'excel']) }}?start_date={{ $startDate->format('Y-m-d') }}&end_date={{ $endDate->format('Y-m-d') }}" 
...
<div class="card shadow-sm border-0 mb-3">
    <div class="card-body">
        <form method="GET" action="{{ route('reports.documents') }}" class="row g-2 align-items-end">
            <div class="col-md-4">
                ...
                @if($byValidationStatus->count() > 0)
                    <div class="table-responsive">
                        <table class="table table-sm">
                            ...
                            <tbody>
                                @foreach($byValidationStatus as $item)
                                    <tr>
                                        <td>
            ...
            <div class="col-md-6">
                <div class="d-flex align-items-center">
                    ...
                    <div class="flex-grow-1 ms-3">
                        <h5 class="mb-0">{{ number_format($avgValidationDelay->avg_delay, 1) }} jours</h5>
